...
class ModerateManager {
    ...
    getAllPosts(sqlInsert){
      let sqlGetAll  = 'SELECT post.post_id, post.title, post.content, post.attachment, DATE_FORMAT(DATE(post.created), \'/%d.%m.%Y\') AS created, post.user_id FROM post JOIN user ON post.user_id = user.user_id ORDER BY post.created DESC;'       
    ...
    getAllComments(){
        let sql = 'SELECT comment.comment_id, comment.content, DATE_FORMAT(DATE(post.created), \'/%d.%m.%Y\') AS created, user.user_id FROM comment JOIN user ON comment.user_id ORDER BY created DESC';
        return new Promise((resolve) =>{
          db.query(sql, function (err, result, fields){
            if (err) throw err;
            resolve(result);
