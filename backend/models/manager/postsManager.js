...
class PostsManager {
  ...
  getAllPosts(sqlInsert) {
    let sqlGetAll  = 'SELECT post.post_id, post.title, post.content, post.attachment, DATE_FORMAT(DATE(post.created), \'/%d.%m.%Y\') AS created FROM post JOIN user ON post.user_id = user.user_id ORDER BY post.created DESC;'       
